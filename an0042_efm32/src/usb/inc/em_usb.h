...
__ramfunc void  USBTIMER_DelayMs(       uint32_t msec );
...
__ramfunc int       USBD_AbortTransfer(     int epAddr );
void                USBD_Connect(           void );
__ramfunc void      USBD_Disconnect(        void );
bool                USBD_EpIsBusy(          int epAddr );
__ramfunc USBD_State_TypeDef  USBD_GetUsbState( void );
const char *        USBD_GetUsbStateName(   USBD_State_TypeDef state );
int                 USBD_Init(              const USBD_Init_TypeDef *p );
__ramfunc int       USBD_Read(              int epAddr, void *data, int byteCount, USB_XferCompleteCb_TypeDef callback );
int                 USBD_RemoteWakeup(      void );
bool                USBD_SafeToEnterEM2(    void );
int                 USBD_StallEp(           int epAddr );
__ramfunc void      USBD_Stop(              void );
int                 USBD_UnStallEp(         int epAddr );
__ramfunc int       USBD_Write(             int epAddr, void *data, int byteCount, USB_XferCompleteCb_TypeDef callback );
