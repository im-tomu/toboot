...
__ramfunc void USBDHAL_AbortAllTransfers( USB_Status_TypeDef reason );
...
__ramfunc void USBDHAL_Disconnect( void );
void USBDHAL_AbortAllEps( void );
__ramfunc void USBDHAL_AbortEpIn( USBD_Ep_TypeDef *ep );
__ramfunc void USBDHAL_AbortEpOut( USBD_Ep_TypeDef *ep );

__ramfunc __STATIC_INLINE USB_Status_TypeDef USBDHAL_GetStallStatusEp(
                                             USBD_Ep_TypeDef *ep, uint16_t *halt );
__ramfunc __STATIC_INLINE uint32_t USBDHAL_GetInEpInts( USBD_Ep_TypeDef *ep );
__ramfunc __STATIC_INLINE uint32_t USBDHAL_GetOutEpInts( USBD_Ep_TypeDef *ep );
__ramfunc __STATIC_INLINE void USBDHAL_SetEPDISNAK( USBD_Ep_TypeDef *ep );
...
__ramfunc __STATIC_INLINE void USBHAL_DisableGlobalInt( void )
{
  USB->GAHBCFG &= ~USB_GAHBCFG_GLBLINTRMSK;
}

__ramfunc __STATIC_INLINE void USBHAL_DisablePhyPins( void )
{
  USB->ROUTE = _USB_ROUTE_RESETVALUE;
}

__ramfunc __STATIC_INLINE void USBHAL_DisableUsbInt( void )
{
...
__ramfunc __STATIC_INLINE void USBHAL_FlushTxFifo( uint8_t fifoNum )
{
  USB->GRSTCTL = USB_GRSTCTL_TXFFLSH | ( fifoNum << _USB_GRSTCTL_TXFNUM_SHIFT );
  while ( USB->GRSTCTL & USB_GRSTCTL_TXFFLSH ) {}
}

__ramfunc __STATIC_INLINE uint32_t USBHAL_GetCoreInts( void )
{
  uint32_t retVal;
...
#if defined( USB_DEVICE )
__ramfunc __STATIC_INLINE void USBDHAL_ActivateEp( USBD_Ep_TypeDef *ep, bool forceIdle )
{
...
__ramfunc __STATIC_INLINE void USBDHAL_DeactivateEp( USBD_Ep_TypeDef *ep )
{
  uint32_t daintmask;
...
__ramfunc __STATIC_INLINE void USBDHAL_EnableInts( USBD_Device_TypeDef *dev )
{
  uint32_t mask;
...
__ramfunc __STATIC_INLINE void USBDHAL_Ep0Activate( uint32_t ep0mps )
{
...
__ramfunc __STATIC_INLINE bool USBDHAL_EpIsStalled( USBD_Ep_TypeDef *ep )
{
  bool retVal = false;
...
__ramfunc __STATIC_INLINE uint32_t USBDHAL_GetAllInEpInts( void )
{
  uint32_t retVal;
...
__ramfunc __STATIC_INLINE uint32_t USBDHAL_GetAllOutEpInts( void )
{
  uint32_t retVal;
...
__ramfunc __STATIC_INLINE void USBDHAL_ReenableEp0Setup( USBD_Device_TypeDef *dev )
...
__ramfunc __STATIC_INLINE void USBDHAL_SetAddr( uint8_t addr )
{
  USB->DCFG = ( USB->DCFG                    &
...
__ramfunc __STATIC_INLINE void USBDHAL_SetEp0InDmaPtr( uint8_t* addr )
{
  USB->DIEP0DMAADDR = (uint32_t)addr;
}

__ramfunc __STATIC_INLINE void USBDHAL_SetEp0OutDmaPtr( uint8_t* addr )
{
...
__ramfunc __STATIC_INLINE USB_Status_TypeDef USBDHAL_StallEp( USBD_Ep_TypeDef *ep )
{
...
__ramfunc __STATIC_INLINE void USBDHAL_StartEp0In( uint32_t len, uint32_t ep0mps )
{
...
__ramfunc __STATIC_INLINE void USBDHAL_StartEp0Out( uint32_t len, uint32_t ep0mps )
{
...
__ramfunc __STATIC_INLINE void USBDHAL_StartEp0Setup( USBD_Device_TypeDef *dev )
{
  dev->ep[ 0 ].in = false;
...
__ramfunc __STATIC_INLINE void USBDHAL_StartEpIn( USBD_Ep_TypeDef *ep )
{
...
#if ( ( USB_FS_BULK_EP_MAXSIZE != 64 ) || ( USB_FS_INTR_EP_MAXSIZE != 64 ) )
#error "Illegal optimization."
#else
    pktcnt = ( ep->remaining - 1 + ep->packetSize ) >> 6;
#endif
#endif
...
__ramfunc __STATIC_INLINE void USBDHAL_StartEpOut( USBD_Ep_TypeDef *ep )
{
  uint32_t pktcnt, xfersize;
...
__ramfunc __STATIC_INLINE USB_Status_TypeDef USBDHAL_UnStallEp( USBD_Ep_TypeDef *ep )
{
  uint32_t depctl, eptype;
  USB_Status_TypeDef retVal = USB_STATUS_REQ_ERR;
