...
__ramfunc __STATIC_INLINE void USBD_ArmEp0( USBD_Ep_TypeDef *ep );
__ramfunc __STATIC_INLINE void USBD_ArmEpN( USBD_Ep_TypeDef *ep );
__ramfunc __STATIC_INLINE void USBD_AbortEp( USBD_Ep_TypeDef *ep );

__ramfunc void USBD_SetUsbState( USBD_State_TypeDef newState );

__ramfunc int  USBDCH9_SetupCmd( USBD_Device_TypeDef *device );

__ramfunc void USBDEP_Ep0Handler( USBD_Device_TypeDef *device );
__ramfunc void USBDEP_EpHandler( uint8_t epAddr );

__ramfunc __STATIC_INLINE void USBD_ActivateAllEps( bool forceIdle )
{
  int i;
...
__ramfunc __STATIC_INLINE void USBD_ArmEp( USBD_Ep_TypeDef *ep )
{
  if ( ep->num == 0 )
  {
    USBD_ArmEp0( ep );
...
__ramfunc __STATIC_INLINE void USBD_DeactivateAllEps( USB_Status_TypeDef reason )
{
  int i;
  USBD_Ep_TypeDef *ep;
...
__ramfunc __STATIC_INLINE USBD_Ep_TypeDef *USBD_GetEpFromAddr( uint8_t epAddr )
{
  int epIndex;
...
__ramfunc __STATIC_INLINE void USBD_ReArmEp0( USBD_Ep_TypeDef *ep )
{
  if ( ep->in )
  {
    USBDHAL_StartEp0In( EFM32_MIN( ep->remaining, ep->packetSize ),
